...
def initialize(dll_directories=_default_dll_directories):
    ...
    prev = os.getcwd()
    exceptions = []
    dll_directories = [os.path.normpath(
        os.path.abspath(d)) for d in dll_directories]
...
def wait_for_any_stream(streams, timeout=None):
    ...
    for i, s in enumerate(streams):
        arr[i] = s._handle
    try:
        c_api.oniWaitForAnyStream(
            arr, len(streams), ctypes.byref(ready_stream_index), timeout)
...
class SensorInfo(object):
    def __init__(self, info):
        ...
        self.videoModes = [info.pSupportedVideoModes[i]
                           for i in range(info.numSupportedVideoModes)]
...
class PlaybackSupport(object):
    ...
    def set_repeat_enabled(self, enable):
        self.device.set_property(
            c_api.ONI_DEVICE_PROPERTY_PLAYBACK_REPEAT_ENABLED, enable)
...
class Device(HandleObject):
    ...
    def _reopen(self):
        ...
        if self._mode:
            c_api.oniDeviceOpenEx(self._orig_uri, self._mode,
    ...
    def get_property(self, property_id, rettype):
        ...
        c_api.oniDeviceGetProperty(
            self._handle, property_id, ctypes.byref(ret), ctypes.byref(size))
        return ret
    ...
    def set_property(self, property_id, obj, size=None):
        ...
        c_api.oniDeviceSetProperty(
            self._handle, property_id, ctypes.byref(obj), size)
    ...
    def invoke(self, command_id, data, size=None):
        ...
        c_api.oniDeviceInvoke(self._handle, command_id,
    ...
    depth_color_sync = property(
        get_depth_color_sync_enabled, set_depth_color_sync_enabled)
...
class CameraSettings(object):
    ...
    def set_auto_exposure(self, enabled):
        self.stream.set_property(
            c_api.ONI_STREAM_PROPERTY_AUTO_EXPOSURE, enabled)
    ...
    auto_white_balance = property(
        get_auto_white_balance, set_auto_white_balance)
...
class VideoStream(HandleObject):
    def __init__(self, device, sensor_type):
        ...
        c_api.oniDeviceCreateStream(
            self.device._handle, sensor_type, ctypes.byref(handle))
    ...
    def register_new_frame_listener(self, callback):
        ...
        c_api.oniStreamRegisterNewFrameCallback(
            self._handle, cbobj, None, ctypes.byref(cb_handle))
    ...
    def configure_mode(self, width, height, fps, pixel_format):
        ...
        mode = VideoMode(resolutionX=width, resolutionY=height,
                         fps=fps, pixelFormat=pixel_format)
    ...
    def set_cropping(self, originX, originY, width, height):
        cropping = c_api.OniCropping(
            enabled=True, originX=originX, originY=originY, width=width, height=height)
    ...
    def reset_cropping(self):
        self.set_property(c_api.ONI_STREAM_PROPERTY_CROPPING,
                          c_api.OniCropping(enabled=False))
    ...
    def set_frame_buffers_allocator(self, allocator=None):
        if not allocator:
            c_api.oniStreamSetFrameBuffersAllocator(
...
class FrameAllocator(object):
    def __init__(self):
        ...
        self._alloc_callback = c_api.OniFrameAllocBufferCallback(
            self._allocate_frame_buffer_callback)
        self._free_callback = c_api.OniFrameFreeBufferCallback(
...
class Recorder(HandleObject):
    ...
    def attach(self, stream, allow_lossy_compression=False):
        c_api.oniRecorderAttachStream(
            self._handle, stream._handle, allow_lossy_compression)
...
class DeviceListener(HandleObject):
    def __init__(self):
        ...
        c_api.oniRegisterDeviceCallbacks(
            self._callbacks, None, ctypes.byref(handle))
        HandleObject.__init__(self, handle)
